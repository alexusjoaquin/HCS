...
const barangay = extractBarangay(username); // Extract barangay
  ...
  const handleFullNameClick = (fullName) => {
    ...
    if (resident) {
        ...
        setViewModalOpen(true); // This is correct
    } else {
        MySwal.fire({
            icon: 'warning',
            title: 'Not Found',
  ...
  const handleFileUpload = async (event) => {
    ...
    try {
      ...
      const s3Params = {
        ...
        Key: `medicines/${file.name}`, // Store the file under the 'medicines' folder
        Body: file,
      ...
      const requiredColumns = ['TransactionID', 'FullName', 'Address', 'MedicineName'];
      ...
      for (const transaction of newTransactions) {
        await medicineService.importMedicinesCSV(transaction); // Add each transaction
      }
      ...
      MySwal.fire({
        ...
        text: 'All medicine data imported successfully!',
        confirmButtonText: 'OK',
      });
...
{isViewModalOpen && selectedResident && (
    <ResidentViewModal
        ...
        onClose={() => setViewModalOpen(false)} // This is correct
